...
def download_and_process_video(video_data: Dict, output_dir: str):
    ...
    for clip_idx in range(len(video_data['intervals'])):
        ...
        if os.path.isfile(clip_path):
            print(f"HAVE CLIPED THE VIDEO: {clip_path}!")
            crop_success = True
        else:
...
def download_video(video_id, download_path, video_format="bestvideo+bestaudio", log_file=None):
    ...
    ydl_opts = {
        ...
        'min_sleep_interval': 3,                #  Number of seconds to sleep before each download.
    ...
    if os.path.isfile(download_path):
        print(f"HAVE DONWLOAED THE VIDEO: {download_path}!")
        success = True
    else:
        ...
        try:
            ...
            print("SUCCESS!")
            time.sleep(10) # avoid YouTube restrict ip
            success = True
        except Exception as e:
            success = False
            print(f"{Fore.RED} ERROR: Failed to download {video_id}. Exception: {str(e)}\n")
            if log_file:
